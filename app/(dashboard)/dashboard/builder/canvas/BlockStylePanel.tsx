  ...
  return (
    <div className="w-[300px] flex-shrink-0 bg-white border-l border-gray-200 flex flex-col overflow-hidden">
      <div className="px-4 py-2.5 border-b border-gray-200 flex items-center justify-between flex-shrink-0">
        <div>
          <p className="text-[12px] font-bold text-gray-500 uppercase tracking-wide">{LABELS[node.type] || node.type}</p>
      ...
      <div className="flex border-b border-gray-200 flex-shrink-0">
        {(!estConteneur ? [["contenu", "Contenu"], ["style", "Style"], ["avance", "Avancé"]] : [["style", "Style"], ["avance", "Avancé"]]).map(([id, label]) => (
          <button key={id} onClick={() => setTab(id as Tab)} className={`flex-1 py-2 text-[13px] font-semibold transition-colors ${tab === id ? "text-[#F5A623] border-b-2 border-[#F5A623]" : "text-gray-400 hover:text-gray-600"}`}>
            {label}
          </button>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin p-3 space-y-4">
        {tab === "contenu" && node.type === "embed-html" && (
          <p className="text-[13px] text-gray-500 leading-relaxed bg-gray-50 border border-gray-200 rounded-lg p-3">
          ...
          <div className="space-y-3">
            <Field label="Classe CSS personnalisée">
              <input type="text" value={style.customClass || ""} onChange={(e) => onChangeStyle({ customClass: e.target.value })}
                className="w-full px-2 py-1.5 text-sm rounded-md border border-gray-200 focus:border-[#F5A623] outline-none" placeholder="ma-classe" />
            </Field>
                ...
                {([["desktop", Monitor], ["tablet", Tablet], ["mobile", Smartphone]] as [keyof typeof visibility, any][]).map(([bp, Icon]) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-1 py-1.5 rounded-md border text-[12px] font-medium transition-colors ${visible ? "border-gray-200 text-gray-600 hover:border-gray-300" : "border-red-200 bg-red-50 text-red-500"}`}
                    >
                      <Icon size={12} />
                    </button>
                  );
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={onDuplicate} className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-600 hover:border-gray-300">
                <Copy size={12} /> Dupliquer
              </button>
              <button onClick={onDelete} className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg border border-red-200 text-sm font-medium text-red-500 hover:bg-red-50">
  ...
  return (
    <div className="space-y-3">
          ...
          <Field key={key} label={labelChamp(key)}>
            {typeof value === "boolean" ? (
              ...
              <select value={value as string} onChange={(e) => onChange({ [key]: e.target.value })}
                className="w-full px-2 py-1.5 text-sm rounded-md border border-gray-200 focus:border-[#F5A623] outline-none">
                {options.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
            ) : CHAMPS_LONG_TEXTE.has(key) ? (
              <textarea value={value as string} onChange={(e) => onChange({ [key]: e.target.value })} rows={3}
                className="w-full px-2 py-1.5 text-sm rounded-md border border-gray-200 focus:border-[#F5A623] outline-none resize-none" />
            ) : (
      ...
      {simples.length === 0 && complexes.length === 0 && (
        <p className="text-[13px] text-gray-400">Ce bloc n'a pas de champ de contenu.</p>
      )}
    </div>
  );
}
...
function StyleEditor({ style, onChange }: { style: BlockStyleOverrides; onChange: (patch: Partial<BlockStyleOverrides>) => void }) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <p className="text-[12px] font-bold text-gray-500 uppercase tracking-wide mb-2">Espacement</p>
          ...
          {(["pt", "pb", "pl", "pr", "mt", "mb"] as const).map((k) => (
            <Field key={k} label={{ pt: "Haut (padding)", pb: "Bas (padding)", pl: "Gauche (padding)", pr: "Droite (padding)", mt: "Haut (marge)", mb: "Bas (marge)" }[k]}>
              <input type="text" value={spacing[k] || ""} onChange={(e) => onChange({ spacing: { ...spacing, [k]: e.target.value } })}
                placeholder="24px" className="w-full px-2 py-1.5 text-sm rounded-md border border-gray-200 focus:border-[#F5A623] outline-none" />
            </Field>
          ))}
        </div>
      </div>
      ...
      <div>
        <p className="text-[12px] font-bold text-gray-500 uppercase tracking-wide mb-2">Fond</p>
        <div className="space-y-2">
          ...
          <Field label="Image (URL)">
            <input type="text" value={background.image || ""} onChange={(e) => onChange({ background: { ...background, image: e.target.value } })}
              placeholder="https://..." className="w-full px-2 py-1.5 text-sm rounded-md border border-gray-200 focus:border-[#F5A623] outline-none" />
          </Field>
          <Field label="Dégradé CSS (prioritaire sur couleur)">
            <input type="text" value={background.gradient || ""} onChange={(e) => onChange({ background: { ...background, gradient: e.target.value } })}
              placeholder="linear-gradient(...)" className="w-full px-2 py-1.5 text-sm rounded-md border border-gray-200 focus:border-[#F5A623] outline-none" />
          </Field>
        </div>
      </div>

      <div>
        <p className="text-[12px] font-bold text-gray-500 uppercase tracking-wide mb-2">Typographie</p>
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Field label="Taille">
              <input type="text" value={typography.taille || ""} onChange={(e) => onChange({ typography: { ...typography, taille: e.target.value } })}
                placeholder="16px" className="w-full px-2 py-1.5 text-sm rounded-md border border-gray-200 focus:border-[#F5A623] outline-none" />
            </Field>
      ...
      <div>
        <p className="text-[12px] font-bold text-gray-500 uppercase tracking-wide mb-2">Bordure</p>
        <div className="grid grid-cols-2 gap-2">
          <Field label="Rayon">
            <input type="text" value={border.radius || ""} onChange={(e) => onChange({ border: { ...border, radius: e.target.value } })}
              placeholder="12px" className="w-full px-2 py-1.5 text-sm rounded-md border border-gray-200 focus:border-[#F5A623] outline-none" />
          </Field>
          <Field label="Épaisseur">
            <input type="text" value={border.width || ""} onChange={(e) => onChange({ border: { ...border, width: e.target.value } })}
              placeholder="1px" className="w-full px-2 py-1.5 text-sm rounded-md border border-gray-200 focus:border-[#F5A623] outline-none" />
          </Field>
        </div>
        <Field label="Couleur">
      ...
      <Field label="Largeur (colonnes dans une ligne)">
        <input type="text" value={style.width || ""} onChange={(e) => onChange({ width: e.target.value })}
          placeholder="50%" className="w-full px-2 py-1.5 text-sm rounded-md border border-gray-200 focus:border-[#F5A623] outline-none" />
      </Field>
    </div>
  );
}

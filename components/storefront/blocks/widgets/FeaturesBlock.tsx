...
export function FeaturesBlock({ id, config, colors: c }: BlockRenderProps) {
  return (
    <section data-axs-id={id} className="py-16 @min-[640px]:py-20 max-w-7xl mx-auto px-4 @min-[640px]:px-6 @min-[1024px]:px-8">
      {config?.titre && (
        <h2 className="text-3xl font-bold font-playfair text-center mb-12" style={{ color: c.texte }}>{config.titre}</h2>
      )}
      <div className="grid grid-cols-1 @min-[640px]:grid-cols-3 gap-8">
